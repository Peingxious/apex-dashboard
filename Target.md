...
| **TodoPlus** | 镜像外部清单 | 卡片指向 vault 内任意 `[[笔记#标题]]`，实时渲染该标题下的 checklist；UI 与 Todo 完全一致（复选框列表 / 添加输入 / 进度条 / 隐藏已完成眼按钮）；勾选/添加/删除/编辑全部通过 `vault.process` 同步回原笔记（仅动 heading 切片内对应行）。数据源：metadataCache + vault.cachedRead，无新持久化层。磁盘格式：`- [[笔记#标题]]` 一行 + 元数据（cover/width/size/grid），不写 `type: todoplus`（列的 `sectionType: todoplus` 是唯一来源），不写 `sourceLink: "[[...]]"`（标题本身就是 source link）。加卡：列头 `+` 按钮打开 DocSearchModal 搜 vault 笔记（substring 过滤 basename/path，最多 20 条）选一条作为来源，默认镜像 `## To-do`（缺则自动追加），不要求笔记预先有 heading |
...
| 版本   | 日期       | 主要变化                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.4.4  | 2026-06-14 | 修复 TodoPlus 标题双 `[[ ]]` 包裹（v1.4.3 的 `addTodoPlusCardFromNote` 用了 `pathToWikiLink(file.path)`，而 `pathToWikiLink` 本就返回 `[[basename]]`，再外面包一层 `[[...#...]]` 就成了 `[[[[dash03]]#To-do]]`）—— 改用 `file.basename`（TFile 自带 `.md` 已剥），恢复单层包裹 `[[dash03#To-do]]`；移除添加分区下拉里的"笔记(无封面)"选项（与 `projects` 完全同形、无独立图标/样式/行为，是历史别名；下拉现为 Notes / Todo / Memo / Library / Todoplus 5 项；已有 `sectionType: "notes"` 分区照常 parse/render/serialize） |
| 1.4.3  | 2026-06-14 | TodoPlus 列头 `+` 按钮换成打开 `DocSearchModal`（复用了 Project 笔记搜索 modal）：substring 过滤 vault 笔记 basename/path（最多 20 条），点击选一条作为 mirror 来源；不要求笔记预先有 `## To-do` heading（缺则 `vault.process` 自动追加），所以"即使没有 To-do 标题也可以添加"——搜索框本身就是"指定的部分笔记"筛选器。`renderer.ts` 新增 `openTodoPlusNoteSearchModal` + `addTodoPlusCardFromNote(column, file, ...)`，移除旧的 `addTodoPlusCard(column, rawInput, ...)`（wikilink-form 解析）                             |
| 1.4.2  | 2026-06-14 | 移除 TodoPlus 卡片冗余元数据：不再写 `type: todoplus`（列的 `sectionType: todoplus` 唯一来源，parseColumns 推导 card.type）和 `sourceLink: "[[...]]"`（标题本身就是 source link，渲染时 `getTodoPlusSourceLinkFromTitle` 解析）；磁盘上 TodoPlus 卡片只剩 `- [[note#heading]]` 一行 + cover/width/size/grid；`DashboardCard.sourceLink` 字段从类型移除，`onCardAdd` option 改 `title`；`extractSourceLink` / `detectCardType` 里的 `metadata.type === "todoplus"` 分支删除；旧文件下次保存自动清理冗余行                   |
| 1.4.1  | 2026-06-14 | TodoPlus 视觉/操作与普通 Todo 卡片完全一致：去掉"Source: [[…]]" 头和"## heading"提示行，复用 `dashboard-task-list` / `dashboard-task-item` / `dashboard-task-add` / `dashboard-progress` DOM；新增 `addTodoPlusItem` / `removeTodoPlusItem` / `editTodoPlusItem` 三个 `vault.process` 写入辅助函数（添加/删除/编辑全部同步回原笔记 heading 切片）；`isTask` / `isTaskCard` 判定扩展 `sectionType === "todoplus"`，让 TodoPlus 卡片也获得眼按钮和无编辑铅笔；CSS 复用 Todo 的 flex/scroll 布局                              |
| 1.4.0  | 2026-06-14 | 新增分区类型 TodoPlus（待办Plus）：卡片指向 `[[笔记#标题]]`，实时渲染该标题下的 checklist（heading 位置切片，metadataCache + vault.cachedRead）；勾选通过 vault.process 同步回原笔记（仅改一行）；md 持久化 `sourceLink: "[[note#heading]]"`；加卡 UI 仿 Project addGroup（input 接受 `[[note#heading]]` / `[[note]]` / `note`，缺标题时自动追加 `## heading`）；新增分区按钮 + 分区 header 切换类型下拉菜单均加入「待办Plus」选项（图标 list-checks），与原有 6 种类型并列                                                |
