...
export function queryVaultFiles(
  app: App,
...
function evaluateFilter(
  ...
  cache: ReturnType<
    typeof import("obsidian").App.prototype.metadataCache.getFileCache
  >,
): boolean {
  ...
  if (prop === "modified" || prop === "created") {
    ...
    if (filter.dateRange) {
      if (filter.dateRange.start && dateStr < filter.dateRange.start)
        return false;
  ...
  if (prop === "path") {
    return filter.values.some((v) =>
      file.path.toLowerCase().includes(v.toLowerCase()),
    );
  }
...
function formatDate(ts: number): string {
  ...
  if (diffDays === 0) {
    ...
    if (diffH === 0) {
      ...
      return diffM <= 1
        ? t("recent.justNow")
        : t("recent.minutesAgo", { count: diffM });
    }
...
export function renderLibrarySection(
  ...
  // Toolbar
  const toolbar = sectionContent.createDiv({
    cls: "dashboard-library-toolbar",
  });
  ...
  setIcon(
    sortDirBtn,
    config.sortDesc ? "arrow-down-wide-narrow" : "arrow-up-wide-narrow",
  );

  // View mode toggle
  const viewToggle = toolbar.createDiv({
    cls: "dashboard-library-view-toggle",
  });
  ...
  for (const mode of viewModes) {
    const btn = viewToggle.createDiv({
      cls:
        "dashboard-library-view-btn" +
        (mode === config.viewMode ? " active" : ""),
    });
    setIcon(btn, viewIcons[mode] ?? "file");
    btn.title = t(
      "library.view" + mode.charAt(0).toUpperCase() + mode.slice(1),
    );
    btn.dataset.viewMode = mode;
    btn.addEventListener("click", () => {
      viewToggle
        .querySelectorAll(".dashboard-library-view-btn")
  ...
  let quickProp: "created" | "modified" =
    config.quickDateFilter?.property ?? "created";
  ...
  function applyQuickFilter(): void {
    config.quickDateFilter =
      quickStart || quickEnd
        ? { property: quickProp, start: quickStart, end: quickEnd }
        : undefined;
  ...
  function openPopup(): void {
    ...
    const propRow = filterPopup.createDiv({
      cls: "dashboard-library-quickfilter-row",
    });
    propRow.createDiv({
      cls: "dashboard-library-quickfilter-label",
      text: t("library.filterProperty"),
    });
    const propSelect = propRow.createEl("select", {
      cls: "dashboard-library-filter-popup-prop",
    });
    propSelect.createEl("option", {
      text: t("library.created"),
      attr: { value: "created" },
    });
    propSelect.createEl("option", {
      text: t("library.modified"),
      attr: { value: "modified" },
    });
    ...
    // Date range
    const dateRow = filterPopup.createDiv({
      cls: "dashboard-library-quickfilter-row",
    });
    dateRow.createDiv({
      cls: "dashboard-library-quickfilter-label",
      text: t("library.filterDateRange"),
    });
    const dateWrap = dateRow.createDiv({
      cls: "dashboard-library-filter-popup-dates",
    });
    const startBtn = dateWrap.createEl("button", {
      cls:
        "dashboard-library-filter-date-btn" + (quickStart ? " has-value" : ""),
  ...
  function renderFilterTag(): void {
    ...
    const x = tag.createSpan({
      cls: "dashboard-library-filter-tag-x",
      text: "×",
    });
  ...
  const pageSizeSelect = toolbar.createEl("select", {
    cls: "dashboard-library-page-size",
  });
  ...
  // Content area
  const contentArea = sectionContent.createDiv({
    cls: "dashboard-library-files",
  });

  // Pagination area
  const paginationArea = sectionContent.createDiv({
    cls: "dashboard-library-pagination",
  });
  ...
  function renderContent(currentConfig: LibraryConfig): void {
    ...
    switch (currentConfig.viewMode) {
      case "grid":
        ...
        break;
      case "list":
        renderListView(contentArea, pageResults, app, currentConfig);
        break;
      case "table":
...
function renderPagination(
  ...
  const prevBtn = nav.createDiv({
    cls:
      "dashboard-library-pagination-btn" +
  ...
  if (startPage > 1) {
    const firstBtn = nav.createDiv({
      cls: "dashboard-library-pagination-page",
      text: "1",
    });
    firstBtn.addEventListener("click", () => onPageChange(1));
    if (startPage > 2) {
      nav.createDiv({
        cls: "dashboard-library-pagination-ellipsis",
        text: "...",
      });
    }
  }
  ...
  for (let i = startPage; i <= endPage; i++) {
    const pageBtn = nav.createDiv({
      cls:
        "dashboard-library-pagination-page" +
...
function renderBadgeRow(
  ...
  for (const [key, value] of Object.entries(fm)) {
    ...
    if (Array.isArray(value)) {
      badge.createSpan({
        cls: "dashboard-library-badge-val",
...
function renderGridView(
  ...
  for (const result of results) {
    ...
    if (parts.length > 1) {
      metaRow.createDiv({
        cls: "dashboard-library-card-path",
        text: parts.slice(0, -1).join("/") + "/",
      });
    }
    metaRow.createDiv({
      cls: "dashboard-library-card-date",
      text: formatDate(result.ctime),
    });
...
function renderListView(
  ...
  // Resolve which properties to show as chips
  // - visibleProperties = undefined or empty array → don't show any chips (backward-compatible default)
  // - visibleProperties = [...] → show only those keys
  const visibleProps = config?.visibleProperties ?? [];
  const showChips = visibleProps.length > 0;

  // Helpers to resolve a property value for a given result
  const getPropDisplay = (key: string, r: LibraryFileResult): string | null => {
    if (key === "name") return r.basename;
    if (key === "modified") return formatDate(r.mtime);
    if (key === "created") return formatDate(r.ctime);
    if (key === "path") return r.file.path;
  ...
  for (const result of results) {
    ...
    //   (modified/created are absorbed into the trailing time element to avoid duplication)
    // - otherwise → only show time, no extra values
    const meta = item.createDiv({ cls: "dashboard-library-list-meta" });

    if (showChips) {
      for (const key of visibleProps) {
        // Skip name (already shown as title) and time keys (consolidated into trailing date)
        if (key === "name" || key === "modified" || key === "created") continue;
        const display = getPropDisplay(key, result);
        if (display == null || display === "") continue;
        meta.createSpan({
          cls: "dashboard-library-list-meta-value",
          text: display,
        });
      }
    }

    // Time is always the final element in the meta row
    if (
      visibleProps.includes("created") &&
      !visibleProps.includes("modified")
    ) {
      meta.createSpan({
        cls: "dashboard-library-list-date",
        text: formatDate(result.ctime),
      });
    } else {
      meta.createSpan({
...
function renderTableView(
  ...
  // - visibleProperties is set & non-empty → use it as the explicit column list
  // - otherwise → fall back to the legacy auto-discovery (filters + first 20 frontmatter keys, max 6)
  let columns: string[];
  if (config.visibleProperties && config.visibleProperties.length > 0) {
    columns = [...config.visibleProperties];
  } else {
    ...
    columns = ["name", "modified", ...propKeys];
  }
  ...
  for (const result of results) {
    ...
    for (const col of columns) {
      ...
      if (col === "name") {
        ...
        td.textContent = formatDate(result.ctime);
      } else if (col === "path") {
        td.textContent = result.file.path;
      } else {
...
function renderKanbanView(
  ...
  for (const [groupName, groupResults] of groups) {
    ...
    col.createDiv({
      ...
      text: `${groupName} (${groupResults.length})`,
    });
