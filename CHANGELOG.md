...
- **Banner editor fully restored** — `renderBanner()` was reduced to "background + edit button" in a previous refactor, which silently dropped the quote, author, color, rotation pools, and pin / collapse button. It now renders the full element tree: background image, overlay, centered quote, author line, pencil-style edit button, bookmark-style pin button, and a `dblclick` shortcut. The rotation timers (60 min for quotes, 30 min for images) are wired up with the index persisted to `localStorage` per dashboard file path, so reloading the dashboard no longer snaps back to the first quote / image. The matching `BannerEditModal` exposes every field again: primary `image`, `quote`, `author`, `quoteColor` (with Reset), the `quotes[]` rotation pool (add / remove), the `images[]` rotation pool (add / remove), plus a Cancel button. The view-level `setupBannerBehavior` that was creating a second pin button on top of the new one has been turned into a no-op so the two handlers can no longer fight over the collapsed state
